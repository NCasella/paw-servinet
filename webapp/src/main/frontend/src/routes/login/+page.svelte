...
<div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8">
  <div class="sm:mx-auto sm:w-full sm:max-w-sm">
    <h2 class="mt-10 text-center text-2xl/9 font-bold tracking-tight text-gray-900">{$t ("login")}</h2>
  </div>

  <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm">
    <form on:submit|preventDefault={handleSubmit} class="space-y-6">
      <div>
        <label for="email" class="block text-sm/6 font-medium text-gray-900">{$t ("email")}</label>
        <div class="mt-2">
      ...
      <div>
        <button type="submit" disabled={loading} class="flex w-full justify-center rounded-md bg-indigo-600 px-3 py-1.5 text-sm/6 font-semibold text-white shadow-xs hover:bg-indigo-500 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-indigo-600">{$t ("login.submit")}</button>
      </div>
    </form>
    ...
    <p class="mt-10 text-center text-sm/6 text-gray-500">
      {$t("login.register-1")}
      <a href="#" class="font-semibold text-indigo-600 hover:text-indigo-500 underline">{$t("login.register-2")}</a>
    </p>
  </div>
